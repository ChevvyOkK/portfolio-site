...
const HERO_COPY: Record<Perspective, { title: string; subtitle: string }> = {
  engineer: {
    title: "Проектирую архитектуру и пишу код, который потом не стыдно поддерживать",
    subtitle:
      "Четыре проекта на четырёх разных стеках — от нативного десктопа на C++/Qt до симуляции с эволюционным ИИ на Rust и WebAssembly.",
  },
  recruiter: {
    title: 'Разработчик, который доводит проекты до продакшена, а не до "почти готово"',
    subtitle:
      "Архитектура, база данных, авторизация, деплой, тесты, CI/CD — полный цикл на каждом из четырёх проектов ниже.",
  },
  client: {
  ...
  return (
    <CursorSpotlight className="border-b border-border">
      <Container className="flex flex-col items-center gap-6 py-28 text-center sm:py-36">
        ...
        <p className="max-w-xl text-lg text-muted">{copy.subtitle}</p>
